...
public class TestSceneGenerator : EditorWindow
{
    ...
    private void CreateTestDinos()
    {
        ...
        // On donne directement une vraie couleur UnityEngine.Color
        CreateOneDino("Rex", DietType.Carnivore, new Color(1f, 0.2f, 0.2f), new Vector3(-5, 0, 0), s);
        CreateOneDino("Diplo", DietType.Herbivore, new Color(0.4f, 0.6f, 1f), new Vector3(-5, 2, 0), s);
    }

    private void CreateOneDino(string name, DietType diet, Color col, Vector3 pos, Sprite sprite)
    {
        GameObject go = new GameObject(name);
        go.transform.position = pos;
        var sr = go.AddComponent<SpriteRenderer>();
        sr.sprite = sprite; 
        sr.color = col; // La couleur est appliquée ici au renderer

        go.AddComponent<CircleCollider2D>();
        var dino = go.AddComponent<Dino>();
        
        // On assigne les nouveaux sprites pour pas que le dino devienne invisible
        dino.passiveSprite = sprite;
        dino.idleSprite = sprite;

        DinoProfileSO profile = ScriptableObject.CreateInstance<DinoProfileSO>();
        profile.speciesName = name; 
        profile.diet = diet;
        dino.profile = profile;
    }
